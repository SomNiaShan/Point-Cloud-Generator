function [data, prefix, summary] = generate_point_cloud_CC(params)
%GENERATE_POINT_CLOUD_CC Extended version of generate_point_cloud.
% Adds 'Staircase' lattice type for laser power-depth testing experiments.
%
% Staircase layout:
%   - Depth levels along Y (each row = one Z depth, deep to shallow)
%   - Power columns along X (each column = one fixed power value)
%   - Each cell is a Nx x Ny patch of evenly-spaced points
%   - Within-patch traversal is always row-major (same direction every row)
...
function [data, prefix, summary] = localGenerateStaircaseFull(s)
% Parameters
nDepths    = localPositiveInteger(  localRequireField(s, 'nDepths'),        'Depth count');
zStartUm   = localFiniteScalar(     localRequireField(s, 'zStartUm'),       'Z Start');
zStepUm    = localFiniteScalar(     localRequireField(s, 'zStepUm'),        'Z Step');
if zStepUm == 0
    error('Z Step must be non-zero.');
end

nPowers    = localPositiveInteger(  localRequireField(s, 'nPowers'),        'Power count');
powerStart = localFiniteScalar(     localRequireField(s, 'powerStart'),     'Power start');
powerEnd   = localFiniteScalar(     localRequireField(s, 'powerEnd'),       'Power end');

patchNx    = localPositiveInteger(  localRequireField(s, 'patchNx'),        'Patch Nx');
patchNy    = localPositiveInteger(  localRequireField(s, 'patchNy'),        'Patch Ny');
pitchXUm   = localPositiveScalar(   localRequireField(s, 'patchPitchXUm'), 'Patch Pitch X');
pitchYUm   = localPositiveScalar(   localRequireField(s, 'patchPitchYUm'), 'Patch Pitch Y');
gapXUm     = localNonnegativeScalar(localRequireField(s, 'gapXUm'),        'Gap X');
gapYUm     = localNonnegativeScalar(localRequireField(s, 'gapYUm'),        'Gap Y');
originXUm  = localFiniteScalar(     localFieldOrDefault(s, 'originXUm', 0),'Origin X');
originYUm  = localFiniteScalar(     localFieldOrDefault(s, 'originYUm', 0),'Origin Y');

% Power value for each column, linearly spaced
if nPowers == 1
...
for iDepth = 1:nDepths
    ...
    for iPow = 1:nPowers
        xBase = originXUm + (iPow - 1) * strideX;
        pVal  = powerCols(iPow);
...
prefix = sprintf('%dx%d_stair_dz%s_P%s_to_%s', ...
    nDepths, nPowers, ...
    localCompactNumber(zStepUm), ...
    localCompactNumber(powerStart), ...
    localCompactNumber(powerEnd));
...
summary.powerModeLabel    = sprintf('Per column (%d levels, %s to %s %%)', ...
    nPowers, localCompactNumber(powerStart), localCompactNumber(powerEnd));
summary.layerTraversalLabel = 'Deep to shallow (Z ascending)';
summary.prefix            = prefix;
end

% -------------------------------------------------------------------------
% Lattice generators (unchanged from generate_point_cloud)
% -------------------------------------------------------------------------
