function fig = point_cloud_generator_app_impl_CC()
%POINT_CLOUD_GENERATOR_APP_IMPL_CC App version with Staircase lattice support.

appFolder = fileparts(mfilename('fullpath'));
if exist('generate_point_cloud_CC', 'file') ~= 2
    addpath(appFolder);
end
...
% --- Staircase panels (rows 7-12, hidden by default) ---
...
onGenerate();
    ...
    function onLatticeTypeChanged(~, ~)
        ...
        % Hide Power and Ordering panels for staircase (built-in)
        setPanelRow(controlGrid, 2, ui.PowerPanel,    'fit', ~isStaircase);
        setPanelRow(controlGrid, 3, ui.OrderingPanel, 'fit', ~isStaircase);

        if isStaircase
            ui.TraversalNoteLabel.Text = 'Traversal: deep to shallow (Z ascending). Row-major within each patch.';
        else
    ...
    function onGenerate(~, ~)
        try
            ...
            [data, prefix, summary] = generate_point_cloud_CC(params);
